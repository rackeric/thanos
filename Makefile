...
docs: $(MDOX) build
	...
	$(MDOX) fmt --links.localize.address-regex="https://thanos.io/.*" *.md $(MD_FILES_TO_FORMAT)
...
check-docs: $(MDOX) build
	...
	$(MDOX) fmt --check -l --links.validate.config-file=.mdox.validators.yaml --links.localize.address-regex="https://thanos.io/.*" *.md $(MD_FILES_TO_FORMAT)
	@find . -type f -name "*.md" | SED_BIN="$(SED)" xargs scripts/cleanup-white-noise.sh
